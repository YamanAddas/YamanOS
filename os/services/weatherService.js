const WEATHER_STATE_KEY = "yamanosWeatherState";
const WEATHER_CACHE_KEY = "yamanosWeatherCacheV2";
...
export class WeatherService {
  ...
  normalizeState(state) {
    ...
    if (next.currentLocation && typeof next.currentLocation === "object") {
      ...
      const storedLat = localStorage.getItem("yamanosLat");
      const storedLon = localStorage.getItem("yamanosLon");
      const storedCity = localStorage.getItem("yamanosCityManual") || localStorage.getItem("yamanosCityAuto");
  ...
  getState() {
    ...
    const legacyUnit = localStorage.getItem("yamanosTempUnit") === "f" ? "f" : "c";
    const legacyExpanded = localStorage.getItem("yamanosWeatherOpen") === "true";
    const state = {
  ...
  saveState(state) {
    ...
    localStorage.setItem("yamanosWeatherAreas", JSON.stringify(normalized.locations.map((loc) => ({
      label: loc.label,
      query: loc.query,
      lat: loc.lat || null,
      lon: loc.lon || null,
    }))));
    localStorage.setItem("yamanosTempUnit", normalized.unit);
    localStorage.setItem("yamanosWeatherOpen", normalized.expanded ? "true" : "false");
  }

  getLegacyAreas() {
    const raw = localStorage.getItem("yamanosWeatherAreas");
    if (raw) {
      try {
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          ...
          "User-Agent": "YamanOS Weather (local app)",
        },
      });
      if (!response.ok) {
        return null;
