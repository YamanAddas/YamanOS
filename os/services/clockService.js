...
export class ClockService {
  ...
  getLegacyWorldClocks() {
    const raw = localStorage.getItem("yamanosWorldClocks");
    if (!raw) return [];
    try {
  ...
  saveState(state) {
    ...
    localStorage.setItem("yamanosWorldClocks", JSON.stringify(normalized.worldClocks));
  }
  ...
  emitRingState() {
    window.dispatchEvent(
      new CustomEvent("yamanos:alarmringstate", {
  ...
  fireAlarm(alarm, source = "legacy") {
    ...
    window.dispatchEvent(new CustomEvent("yamanos:notify", { detail: { message: `Alarm: ${ring.label}` } }));
    window.dispatchEvent(new CustomEvent("yamanos:alarm", { detail: { alarm, source } }));
  ...
  dismissActiveAlarm() {
    ...
    window.dispatchEvent(new CustomEvent("yamanos:notify", { detail: { message: `${label} dismissed` } }));
    return true;
  }
  ...
  snoozeActiveAlarm(minutesOverride) {
    ...
    this.clearActiveRing();
    if (updated) {
      window.dispatchEvent(new CustomEvent("yamanos:notify", { detail: { message: `${label} snoozed ${snoozeMins}m` } }));
    }
    return updated;
  }
  ...
  updateAlarmSnooze(source, alarmId, snoozeUntil, snoozeMins) {
    ...
    if (source === "v2") {
      ...
      if (changed) {
        ...
        window.dispatchEvent(new CustomEvent("yamanos:clockchange"));
      }
      return changed;
    }
  ...
  fireTimerDone() {
    window.dispatchEvent(new CustomEvent("yamanos:notify", { detail: { message: "Timer complete" } }));
    window.dispatchEvent(new CustomEvent("yamanos:timerdone"));
    this.playAlarmSound();
  }
  ...
  tick() {
    ...
    window.dispatchEvent(new CustomEvent("yamanos:clocktick"));
  }
