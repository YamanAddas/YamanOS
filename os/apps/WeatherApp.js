...
export class WeatherApp extends App {
    ...
    async init() {
        ...
        window.addEventListener('yamanos:weatherchange', this.onWeatherChange);
    }
    ...
    destroy() {
        ...
        if (this.onWeatherChange) window.removeEventListener('yamanos:weatherchange', this.onWeatherChange);
    ...
    loadState() {
        ...
        const raw = localStorage.getItem('yamanos_weather_v1');
        if (raw) {
            try {
    ...
    seedLegacyLocation(state) {
        ...
        try {
            const legacy = JSON.parse(localStorage.getItem('yamanos_weather_v1') || 'null');
    ...
    loadRecentSearches() {
        try {
            const raw = JSON.parse(localStorage.getItem('yamanosWeatherRecentSearchesV1') || '[]');
    ...
    saveRecentSearches() {
        try {
            localStorage.setItem('yamanosWeatherRecentSearchesV1', JSON.stringify((this.recentSearches || []).slice(0, 8)));
        } catch (_) { /* ignore */ }
    }
    ...
    saveState() {
        ...
        if (!current) return;
        localStorage.setItem('yamanos_weather_v1', JSON.stringify({
            city: current.label,
            lat: current.lat,
            lon: current.lon,
    ...
    getClockPrefs() {
        ...
        const v2 = parse('yamanos_clock_v2');
        if (v2 && typeof v2.use24h === 'boolean') return { use24h: v2.use24h };
        const v1 = parse('yamanosClockState');
    ...
    getStoredCoordsLocation() {
        const latRaw = localStorage.getItem('yamanosLat');
        const lonRaw = localStorage.getItem('yamanosLon');
        const lat = Number(latRaw);
        const lon = Number(lonRaw);
        const label = localStorage.getItem('yamanosCityManual') || localStorage.getItem('yamanosCityAuto') || 'Current Location';
    ...
    setPrimaryLocation(item, { refresh = true, emitChange = true, setCurrent = true } = {}) {
        ...
        this.saveState();
        if (emitChange) window.dispatchEvent(new CustomEvent('yamanos:weatherchange'));
        this.renderLocations();
    ...
    notify(message) {
        window.dispatchEvent(new CustomEvent('yamanos:notify', { detail: { message } }));
    }
