...
export class ClockApp extends App {
    constructor(kernel, pid) {
        ...
        this.storeKey = 'yamanos_clock_v3';
    ...
    async init() {
        ...
        window.addEventListener('yamanos:clock_update', this.onClockUpdate);
    }
    ...
    getWeatherCoords() {
        ...
        try {
            const state = JSON.parse(localStorage.getItem('yamanosWeatherState') || 'null');
    ...
    destroy() {
        ...
        this.intervals = [];
        if (this.onClockUpdate) window.removeEventListener('yamanos:clock_update', this.onClockUpdate);
        super.destroy();
    }
}
