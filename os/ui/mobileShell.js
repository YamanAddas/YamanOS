...
export class MobileShell {
  ...
  seedDefaultFolders() {
    try {
      ...
      const grid = this.components.grid;
      const seedFlag = localStorage.getItem('yamanos_mobile_seed_v06');
      ...
      localStorage.setItem('yamanos_mobile_seed_v06', 'true');
    } catch (e) {
  ...
  _applyHomeLayoutDefaults(apps) {
    try {
      const key = 'yamanos_home_layout_v100';
      if (localStorage.getItem(key)) return;

      const mode = localStorage.getItem('yamanos_home_layout_mode') || 'type-name';
      if (mode === 'type-name') {
  ...
  bindAlarmOverlay() {
    ...
    window.addEventListener('yamanos:alarmringstate', (event) => {
