...
export class MobileContextMenu {
  constructor(grid) {
    ...
    const saved = localStorage.getItem('yamanos_wallpaper');
  ...
  applySavedWallpaper(saved) {
    ...
    localStorage.setItem('yamanos_wallpaper', `url("${normalized}")`);
  }

  changeWallpaper() {
    const current = this.normalizeWallpaper(localStorage.getItem('yamanos_wallpaper') || '');
    let nextIndex = 0;
  ...
  _gridMenu(menu) {
    this._addIconHeader(menu, '💎', 'YamanOS');
    return [
