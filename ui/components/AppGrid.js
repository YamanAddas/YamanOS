...
export class AppGrid {
  ...
  updateLayoutMetrics() {
    ...
    document.documentElement.style.setProperty('--yamanos-status-bar-height', `${this.currentLayout.statusBarHeight || 44}px`);
    document.documentElement.style.setProperty('--yamanos-search-top', `${search.top ?? 56}px`);
    document.documentElement.style.setProperty('--yamanos-search-height', `${search.height ?? 50}px`);
    document.documentElement.style.setProperty('--yamanos-search-width', `${search.width ?? Math.min(window.innerWidth * 0.92, 620)}px`);
