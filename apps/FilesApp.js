...
const FILES_ORDER_KEY = 'yamanos_files_order_v1';
...
export class FilesApp extends App {
    ...
    async updateStorageBadge() {
        ...
        this.storageBadge.title = `YamanOS Files: ${this._formatBytes(info.fsBytes)} used`;
    }
    ...
    async showStorageInfo() {
        ...
        const lines = [
            'YamanOS Files Storage',
            '',
            `Saved location: browser localStorage (${this.fs?.prefix || 'yamanos:fs:'}*)`,
            `Files + folders tracked: ${info.itemCount}`,
            `YamanOS Files usage: ${this._formatBytes(info.fsBytes)}`,
    ...
    async getStorageInfo() {
        const fsPrefix = this.fs?.prefix || 'yamanos:fs:';
